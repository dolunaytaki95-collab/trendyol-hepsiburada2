...
def get_categories():
    """
    Fetch ALL usable leaf/active/available categories and cache them.
    Old 500-item caches are intentionally ignored so an incomplete cache
    can never block category matching for new products.
    """
    if CATEGORY_CACHE.exists():
        try:
            ...
            # The previous version stored only the first 500 categories.
            # Never trust that incomplete cache.
            if len(categories) >= 1000:
                log(f"📚 Kategori önbelleği kullanılıyor: {len(categories)} kategori")
                return categories
            log(f"♻️ Eksik eski kategori önbelleği bulundu ({len(categories)}); yeniden çekilecek.")
        except Exception:
            pass
    ...
    # Hepsiburada documents page/size pagination. We keep requesting pages
    # until the response tells us it is the last page, or returns fewer rows.
    for _ in range(100):
        params = dict(base_params)
        params["page"] = page

        response = hb_request("GET", url, params=params)
        log(f"📚 HB kategori | HTTP {response.status_code} | page={page} size=2000")
        ...
        page_data = body.get("data") if isinstance(body, dict) else body

        meta = body if isinstance(body, dict) else {}
        raw = page_data

        if isinstance(page_data, dict):
            # Depending on the API serializer, rows can be under content/items/data.
            ...
            # Some responses keep pagination metadata inside data.
            for key in ("totalPages", "totalElements", "numberOfElements", "last"):
                if key in page_data:
                    meta = page_data
                    break
        ...
        page_rows = [x for x in raw if isinstance(x, dict)]

        if not page_rows:
            if page == 0:
                raise RuntimeError("HB kategori HTTP 200 fakat kategori listesi boş döndü.")
            break

        categories.extend(page_rows)
    ...
    if not categories:
        raise RuntimeError("HB kategori servisi veri döndürmedi.")

    CATEGORY_CACHE.write_text(
        json.dumps(
            {"version": 2, "categories": categories},
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
...
def choose_category(product, categories):
    """
    Match a Trendyol category to one and only one usable HB leaf category.

    Priority:
      1) explicit Trendyol category ID, if it exists in the HB list
      2) exact normalized category name
      3) exact normalized path/name token set
      4) safe product-type rules for common jewelry categories
    Never choose an arbitrary fuzzy winner.
    """
    ...
    for c in categories:
        ...
        if leaf is False or available is False or status in {"aktif degil", "active degil"}:
            continue
        usable.append(c)

    if target_id is not None:
        direct = [c for c in usable if extract_category_id(c) == target_id]
        if len(direct) == 1:
            return direct[0]
    ...
    # Exact target tokens against category name/path.
    ...
    for c in usable:
        ...
        if target_tokens and target_tokens.issubset(tokens):
            exact_token_matches.append(c)

    # If exactly one category contains every target token, accept it.
    if len(exact_token_matches) == 1:
        return exact_token_matches[0]

    # Safe common-jewelry fallbacks. These still require a UNIQUE result.
    token_rules = [
        ("piercing", ("piercing",)),
        ("sahmeran", ("sahmeran",)),
        ("bileklik", ("bileklik",)),
    ...
    for trigger, required_tokens in token_rules:
        if trigger not in target:
            continue
        matches = []
        for c in usable:
            hay = norm(f"{c.get('name', '')} {c.get('paths', '')}")
            if all(token in hay.split() for token in required_tokens):
                # Prefer categories whose material/type tokens agree with the target.
                target_materials = {
                    t for t in ("celik", "gumus", "altin", "bijuteri")
                    if t in target.split()
                }
    ...
    # Final fuzzy match is only accepted if one result is overwhelmingly unique
    # AND shares at least two target tokens or 100% of the target tokens.
    scored = []
    for c in usable:
        name_norm = norm(c.get("name"))
        path_norm = norm(c.get("paths"))
        hay = f"{name_norm} {path_norm}"
        hay_tokens = set(hay.split())
        overlap = len(target_tokens & hay_tokens) / max(1, len(target_tokens))
        score = SequenceMatcher(None, target, name_norm).ratio()
        if (overlap >= 0.75 and score >= 0.70) or (overlap == 1.0 and score >= 0.50):
    ...
    if len(scored) == 1:
        return scored[0][2]
    if len(scored) >= 2:
        if scored[0][1] > scored[1][1] or (
            scored[0][1] == scored[1][1] and scored[0][0] - scored[1][0] >= 0.15
        ):
            return scored[0][2]
    return None
