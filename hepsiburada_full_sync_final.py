...
def get_categories():
    ...
    response = hb_request(
        "GET",
        f"{HB_CATALOG_BASE}/product/api/categories/get-all-categories",
        params={"page": 0, "size": 2000, "leaf": "true", "status": "active", "available": "true"},
    )
    if response.status_code != 200:
        raise RuntimeError(f"HB kategori HTTP {response.status_code}: {response.text[:5000]}")
    body = json_or_fail(response, "HB kategori")
    categories = body.get("data") if isinstance(body, dict) else []
    if not isinstance(categories, list):
        categories = []
    CATEGORY_CACHE.write_text(json.dumps(categories, ensure_ascii=False, indent=2), encoding="utf-8")
...
def choose_category(product, categories):
    ...
    if not target:
        return None
    exact = [c for c in categories if norm(c.get("name")) == target]
    if len(exact) == 1:
        return exact[0]
...
def process_new_products(new_products, state):
    ...
    for product in new_products:
        ...
        if not category:
            log(f"⏸️ Yeni ürün gönderilmedi: güvenli kategori eşleşmesi bulunamadı | {product['title']} | kategori={product['categoryName']}")
